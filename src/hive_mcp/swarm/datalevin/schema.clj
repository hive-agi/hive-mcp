(ns hive-mcp.swarm.datalevin.schema
  "Datalevin schema definitions for swarm hivemind coordination.
   ...
   Compat shim: moved to hive-agent.swarm.datalevin.schema in hive-agent. Every public var delegates there
...
(defn- impl [sym]
  (requiring-resolve (symbol "hive-agent.swarm.datalevin.schema" (name sym))))
...
(def claude-model? @(impl 'claude-model?))
