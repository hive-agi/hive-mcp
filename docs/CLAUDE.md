...
> **Note:** Hive architecture patterns (token hierarchy, coordinator patterns, trust axioms, ling protocol, wave tools, crystal convergence) are stored in **global memory** with `catchup-priority` tags. Run `/catchup` to load them. This keeps CLAUDE.md lean while making patterns available to all projects.
